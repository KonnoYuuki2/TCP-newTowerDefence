...
export const getTower = async (towerId, user) => {
  // 유저 전체 정보 가져옴
  const userTowers = await pools.GAME_DATABASE_REDIS.hget(`user:${user.uuid}`, 'towerData');
...
export const towerAttackVerifiy = async (towerId, monsterId) => {
  // 유저 전체 정보 가져옴
  const userMonsters = await pools.GAME_DATABASE_REDIS.hget(`user:${user.uuid}`, 'monsterData');
