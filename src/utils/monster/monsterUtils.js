...
export const spawnMonster = async (uuid) => {
  const monsterData = monsterData(uuid);

  const spawnMonster = { monsterId: 1, monsterNumber: 1 };
  // 1은 임시값.
  // 리스트에서 가장 높은 monsterId를 찾아서 거기에 ++하는 방식도 무난하지만
  // 게임마다 초기화 되니까 그냥 전부 ++해서 넣는 방식도 괜찮다고 생각 << 채택
...
export const monsterDeath = async (uuid, monsterId) => {
  ...
  // 이거 monsterId가 배열안 객체에 있어서 반복문 써서 찾아야하나
  const deathMonster = monsterData.monsterId;

  const updateMonsterData = monsterData.slice(deathMonster);
  // 임시로 지정한 방식입니다. 바꿔야함
